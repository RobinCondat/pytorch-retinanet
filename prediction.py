...
def main(args=None):
    ...
    """
    test_ids = ['{:06d}'.format(i) for i in range(7518)]
    best_retinanet=None
       
    dataset_val = CSVDataset(train_file=C.simple_label_test_file, class_list=C.csv_classes, channels_ind = C.channels_ind, 
                             ids = test_ids,transform=transforms.Compose([Normalizer(C.channels_ind), Resizer(C.image_min_size)]))
    ...
    mAP = csv_eval.evaluate(dataset_val, best_retinanet,C, save_detection_path = C.result_labels_path)
